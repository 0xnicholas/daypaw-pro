...
**Rely on `bundleDependencies` alone for the CLI.** Rejected: that npm behavior is gone; the tarball's file list is the acceptance criterion, and it requires the `dependencies` pin.
