...
- The SDK smoke consumer pins zod to the repository-resolved version instead of `^4.4.3`. zod changes generic shapes inside semver-compatible ranges, so a floating smoke range breaks on registry drift unrelated to the diff; the SDK's zod contract is the tested version, while the published peer range keeps promising the range.
