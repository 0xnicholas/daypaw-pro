...
弹窗提交重新到达引擎（真实浏览器对重建 bundle 的端到端验证：run 行落库、对话打开）。`@daypaw/web-app` 为契约 spec dev 依赖 `dsh-typert-registry` 与 `dsh-api-gateway`。今后任何 `durable/*` 客户端调用，只要黄金道无法校验其 args 键（fixture 从不拒绝），都需要同样的网关背书契约用例，否则 args 不匹配会无声上线。
