...
- 改道新增的等待全部带该文件的显式 10 秒超时；失败循环里的 `findBy` 原以默认 1 秒等待一个 hosted 硬件可能滞后的挂载。
