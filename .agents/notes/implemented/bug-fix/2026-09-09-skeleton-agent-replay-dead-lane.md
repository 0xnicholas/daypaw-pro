...
The three committed `session.jsonl` goldens were refreshed (`DSH_SNAPSHOT=refresh`) to the v2-normalized format: `seq`/`time` envelopes, folded `stream` arrays replacing `assistant/chunk` events, the `isSeeded` header field, and `sourceEventSeqs` pointing at the new sequence numbers. The event-type order is otherwise unchanged in all three scenarios.
