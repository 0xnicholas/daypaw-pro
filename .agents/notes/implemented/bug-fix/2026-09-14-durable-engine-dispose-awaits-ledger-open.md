...
- Driver abort deliberately keeps its synchronous-prefix position: driving `ready` first admits already-queued sibling teardown (session inbox-projection unregistration) before the abort, and `ReactLoopAgent.cancel` then reads inbox projections the abort dispatch has already removed.

## Testing

- `behavior.spec.ts` gains a regression case that boots the engine, touches nothing on `ctx.durable` (the engine booted with no roster entries), disposes, and asserts the ledger directory is byte-stable across a bounded window and holds no `-wal`/`-shm` companions. The window only bounds the negative observation, it never gates the assertion.
- `agent.spec.ts` reports no unhandled errors under the synchronous-prefix ordering; `pnpm exec vitest run packages/daypaw/engine packages/daypaw/sdk` is 211/211, and the previously failing spec stays green under 8-way CPU load.
...
- The dispose-before-open window leaks no open `DatabaseSync`.
