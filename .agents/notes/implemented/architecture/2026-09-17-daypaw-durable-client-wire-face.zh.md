...
wire 类型在本包声明，绝不 alias `@daypaw/engine`。alias 不可能与自己不一致，因此在边界上什么都证明不了；独立声明才是解析器校验的对象，漂移由执行捕获。漂移由 `@daypaw/web-app` 的活网关 spec 捕获：七个端点全部经 `createDurableClient` 打到真 Typert 网关与真引擎，而组装金样赛道走的是不做描述符校验的 fixture 传输。浏览器平面保持零 `@daypaw/engine` 依赖边。
...
**各消费者 spec 里手写行字面量。** 引擎序列化一动它们也不会红。wire 契约的证明必须在真引擎上跑。

**建 `/testing` 子路径导出共享 fake。** 三个包当前重复同一 fake 形状；现在抽出等于为需求更窄的消费者（第二个消费者只读一个端点）发布测试面。等重复变成第三次再议。

## 后果

- wire 知识只有一个家：四个 `ui-*` 包不持有 `durable/*` 端点字符串、wire 行类型，也不自带 run-status 文案。
- 任务面在英文下为 done 与 error 两态渲染 `Completed` 与 `Failed`（别处仍为 `Done`/`Something went wrong`）。组装金样记录该变化。
- 空 `import type {} from '@daypaw/ui-inbox/client'` 承载 ui-inbox 的 `SlotMap` 合并（槽名靠它定型），故 `@daypaw/ui-agents` 与 `@daypaw/ui-settings` 各保留一处有注释的携带者；删掉即 typecheck 红。模块 augmentation 是程序级全局，在依赖处写明。
- journal 行的 `occurrence` 从 0 起；默认 step key 为 `name#occurrence`。
