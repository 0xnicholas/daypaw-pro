...
- The generation counter has one home; the six store files hold one `LatestLoad` field each and their `load`/`select`/`fetch` bodies now read as read-plus-policy. Observable behavior is unchanged: all 66 store specs pass against the six stores.
