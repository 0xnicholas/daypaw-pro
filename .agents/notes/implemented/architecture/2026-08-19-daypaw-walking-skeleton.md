...
- **Core/service split inside `@daypaw/engine`** — `DurableEngineCore` owns every state transition over a `JournalStore` seam and holds no Cordis and no database handle; the `ctx.durable` `Service` is a thin adapter that opens/migrates the ledger and delegates. The fault-injection suite drives the core through Proxy-wrapped stores (per-method faults), which is the seam's second real consumer and keeps error-path tests free of process theater. The fault-injection suite keeps error paths inside the process; the SIGKILL suite spawns the real example host via `node --import tsx/esm`.
...
- **Boot scan once at construction** — rejected: the registry holds no definitions yet, and revival then needs a second trigger invisible in the API. Registration-triggered scans make "boot scan revives" true exactly when it becomes possible.
...
- Promise rows, `waiting` status, and gate resolution bind to `ctx.waitFor` (issue #47, [gate note](../feature/2026-08-23-durable-gate-waitfor.md)); the timer table and `ctx.sleep` bind to [durable timers](../feature/2026-09-19-daypaw-durable-timer.md); relational invariants become demand-driven future work.
