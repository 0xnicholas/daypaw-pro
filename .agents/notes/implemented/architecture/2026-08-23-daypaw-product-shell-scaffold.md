...
- **Roster placeholder ruling** — the fork `cordis.patch.yml` keeps the complete upstream browser roster as placeholders so the composition boots end to end; removing rows the shell does not ship and swapping in rewritten ones is scoped to the board epics (#56–#60), and `DAYPAW_PROFILE_BUNDLES` stays untouched until the profile wiring epics (#61/#62).
