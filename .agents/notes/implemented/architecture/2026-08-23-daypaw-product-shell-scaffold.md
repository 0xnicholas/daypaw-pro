...
- **`packages/daypaw/web-app` (`@daypaw/web-app`)** — a private glue bundle mirroring `packages/bundle/web-app`. The fork glue differs from `packages/bundle/web-app` in three points: the built index resolves to `@daypaw/web-frontend/dist/index.html`; the surface prompt speaks daypaw context and states that no rebuild watcher is wired for this shell; the URL line prints `daypaw web:`. The glue reads and prints `DSH_WEB_URL`: managed shell variables live in the reserved `DSH_*` namespace (`shell-env` rejects other prefixes at registration and `dsh-subprocess` strips ambient `DSH_*` from child environments), so a `DAYPAW_WEB_URL` name cannot pass that registry (package README, Known Limitations). Observable keys the two bundles never live-load together with keep upstream values: plugin names `web-app`/`web-startup`/`web-app-invariant`, prompt section `app:web-surface`, shellEnv registration `web-runtime`, services `webStartup`/`webRuntime`.
- **Roster placeholder ruling** — the fork `cordis.patch.yml` keeps the upstream browser roster as placeholders so the composition boots end to end; the roster carries the fork's `ui-inbox` sidebar row (issue #55, [shell IA skeleton](../feature/2026-08-24-daypaw-shell-ia-skeleton.md)) alongside upstream placeholder rows, and removing the remaining rows the shell does not ship and swapping in rewritten ones stays scoped to the board epics (#56–#60), with `DAYPAW_PROFILE_BUNDLES` untouched until the profile wiring epics (#61/#62).
...
- `pnpm run build` emits `apps/daypaw-web/dist`, which the glue resolves through package exports; the three mirrored specs (startup provider over a real Loader tree, LAN-trust sampling, runtime glue) cover `packages/daypaw/web-app` per file.
