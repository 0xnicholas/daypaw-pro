...
- **The engine link is healed, not installed**: every launch symlinks the closure-bundled `@daypaw/engine` into the profile's `node_modules` (wrong or dangling links are re-pointed; a real directory fails loud), mirroring the launcher fallback's heal semantics. No pnpm install runs at seed time, keeping the first run offline.
...
- **Adding `daypaw` to `PROFILE_TEMPLATES` (core touch)** — rejected: it would be the first edit to a shipped upstream runtime file (every registered core touch so far is repo configuration or tooling), it breaks the delivery line's zero-core-touch claim, and it becomes a permanent upstream-sync replay item. Its only extra payoff — source-state `pnpm dsh --profile daypaw` auto-initializing — is outside the release acceptance.
