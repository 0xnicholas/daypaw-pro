...
- **The engine link is healed, not installed**: every launch heals the daypaw family flat into the installation fallback (`$DSH_HOME/profiles/node_modules`) from the CLI package's own dependency closure through the launcher's own `healProfilesModuleFallback` anchored on the CLI manifest, which a pnpm operation inside the profile cannot prune. No pnpm install runs at seed time, keeping the first run offline.
- **Coverage**: `packages/daypaw/cli/tests/seed-profile.spec.ts` boots the seeded template through a real Loader composition and asserts the composed engine row mounts and completes a run; the release smoke boots the packed tarball with a bare `daypaw` from a fresh `DSH_HOME` through the URL line and a served dist page, asserting the seeded profile, the fallback links, and the ledger at the launch cwd (shape owned by [the direct-shell note](../feature/2026-08-26-daypaw-direct-shell-delivery.md)). [The delivery note](../process/2026-08-22-daypaw-npm-self-contained-delivery.md) owns the packaging pipeline.

## Alternatives considered

- **Adding `daypaw` to `PROFILE_TEMPLATES` (core touch)** — rejected: it would have been the first edit to a shipped upstream runtime file, it broke the delivery line's zero-core-touch claim, and it became a permanent upstream-sync replay item. It would also have made source-state `pnpm dsh --profile daypaw` auto-initialize, which the release acceptance does not cover. (The heal took the one registered runtime-source exception — see the sharing note above.)
