...
Wire types are declared in the package, never aliased from `@daypaw/engine`. An alias cannot disagree with its source, so it proves nothing at the boundary; an independent declaration is what the parser validates, and drift is caught by execution. Drift is caught by the live-gateway spec in `@daypaw/web-app`: it drives all seven endpoints through `createDurableClient` against the real Typert gateway and a real engine, while the assembled golden lane answers from a fixture transport that performs no descriptor validation. The browser plane keeps zero `@daypaw/engine` dependency edges.
...
**Hand-written row literals in each consumer's spec.** They cannot fail when the engine's serialization moves. The wire contract's proof has to run against the engine.
...
- Wire knowledge has one home: the four `ui-*` packages hold no `durable/*` endpoint strings, no wire row types, and no run-status copy of their own.
- Task surfaces render `Completed` and `Failed` for the done and error states in English (`Done`/`Something went wrong` elsewhere). The assembled golden records it.
- The empty `import type {} from '@daypaw/ui-inbox/client'` carries the ui-inbox `SlotMap` merge that types slot names, so `@daypaw/ui-agents` and `@daypaw/ui-settings` each keep one documented carrier; deleting it fails typecheck. Module augmentations are program-global, stated where they are relied on.
- A journal row's `occurrence` is 0-based, and the default step key is `name#occurrence`.
