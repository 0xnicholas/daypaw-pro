...
- **`@daypaw/cli` 删除内联副本**：`ensureEngineLink` 调用导出的 `ensureSymlink`。
...
- 上游同步仪式（ADR 0001）每次 sync 重放该改动；丢失会大声失败——`@daypaw/cli` 的类型检查对不导出 `ensureSymlink` 的 `dsh-app-boot` 失败——漂移在构建期暴露，绝不静默。
- 发布的 CLI tarball 以上游名义打包 fork 改过的 `@deepseek-ai/dsh-app-boot`；`bundleDependencies` 使其只存在于 tarball 内部，不会与消费者另行安装的上游 npm 副本冲突。模板播种 note 的零 core touch 表述让位于本 note 登记的例外。
