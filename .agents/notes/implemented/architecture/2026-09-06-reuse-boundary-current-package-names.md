...
- **Keep the adjudication text verbatim as a historical record** — rejected: spec 05 is current-state prose; a boundary list whose package names do not resolve on the tree fails every future reader and grep.
- **Re-adjudicate the boundary while migrating** — rejected: wayfinder #80 adjudication 1 kept every cluster verdict ("存量边界全簇维持"); the dual listing of ui-chat (wholesale-reused layers plus rewritten presentation) implements that verdict in current package names.
