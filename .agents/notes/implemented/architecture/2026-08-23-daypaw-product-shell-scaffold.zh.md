...
- **`packages/daypaw/web-app`（`@daypaw/web-app`）**——镜像 `packages/bundle/web-app` 的私有胶水 bundle。fork 胶水与 `packages/bundle/web-app` 有三点不同：构建产物的 index 解析到 `@daypaw/web-frontend/dist/index.html`；surface prompt 用 daypaw 语境并声明本壳未接重建 watcher；URL 行打印 `daypaw web:`。胶水读取并打印 `DSH_WEB_URL`：受管 shell 变量生活在保留的 `DSH_*` 命名空间（`shell-env` 在注册时拒绝其他前缀，`dsh-subprocess` 会从子进程环境剥掉 ambient `DSH_*`），`DAYPAW_WEB_URL` 这个名字因此过不了该注册表（见包 README 的 Known Limitations）。两个 bundle 从不同时加载的可观察键保持上游取值：插件名 `web-app`/`web-startup`/`web-app-invariant`、prompt section `app:web-surface`、shellEnv 注册名 `web-runtime`、服务 `webStartup`/`webRuntime`。
- **roster 占位裁决**——fork 的 `cordis.patch.yml` 把上游浏览器 roster 保留为占位，使组合端到端可启动；名册携带 fork 的 `ui-inbox` 侧栏行（issue #55，[壳 IA 骨架](../feature/2026-08-24-daypaw-shell-ia-skeleton.zh.md)），其余壳不交付的行的移除与重写版换入仍归板块票（#56–#60），`DAYPAW_PROFILE_BUNDLES` 在 profile 接线票（#61/#62）之前不动。
...
duplication 处理：jscpd 门禁只扫 `packages` 和 `scripts`，`apps/daypaw-web` 无需豁免；三个胶水源码是有意镜像，携带注明 fork 载体理由的 `jscpd:ignore-start/end` 块，沿用 `packages/daypaw/*/src/invariant.ts` 先例。`duplication` 门报出一处 clone：`app-boot/profile.ts` vs `daypaw/cli/index.ts`（#47）。
...
- `pnpm run build` 产出 `apps/daypaw-web/dist`，胶水经 package exports 解析它；三个镜像 spec（真实 Loader 树上的 startup provider、LAN trust 采样、运行时胶水）逐文件覆盖 `packages/daypaw/web-app`。
