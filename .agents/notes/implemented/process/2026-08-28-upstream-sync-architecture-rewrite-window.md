...
- **Fork UI shell port** (every fork shell file added in #54–#62): `ctx.slots` → ui-renderer; session standard kit → `useChat`/`useSession`/`useSessionPendingInteraction` (declaration merges); pending approvals → ui-approval's `PendingApproval` class (`answer()` channel); `IApiClient` → `ctx.remote` namespaces (with subpath injects); `host.describe` → `session/modelCatalog`; `agentPreset` row field retired; conversation seat session-maybe → session scope.
