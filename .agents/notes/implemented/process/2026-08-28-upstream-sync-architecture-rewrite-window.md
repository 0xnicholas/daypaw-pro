...
- **Fork UI shell port** (every fork shell file added in #54–#62): `ctx.slots` → ui-renderer; session standard kit → `useChat`/`useSession`/`useSessionPendingInteraction` (declaration merges); pending approvals → ui-approval's `PendingApproval` class (`answer()` channel); `IApiClient` → `ctx.remote` namespaces (with subpath injects); `host.describe` → `session/modelCatalog`; the roster row carries no `agentPreset` field; conversation seat session-maybe → session scope.
...
- **web-app roster**: session-reference, file-reference-local, subagent-model-selection-settings, the session/settings/workspace controllers, ui-renderer, ui-session, ui-approval, ui-chat.
...
- **Docs**: doc-sync fully green (2026-08-28 pairing rules tightened — 687 zh notes got switcher/link alignment, fork corpus excluded via manifest, generated-doc zh synced, AGENTS budget ceiling 1990).
...
- The fork UI shell uses the new upstream client architecture; future syncs in this area are incremental.
- `ensureSymlink` is not a registered core touch; the fork consumes the upstream public `healProfilesModuleFallback`.
- Known environmental baseline red (not sync-introduced): `packages/session/session-projection-cache/tests/cache.spec.ts` (cold-read and write-policy cases) flakes under node 26 — at the synced upstream commit it is red under node 26 and green under node 23.11 and node 24 (CI). `hooks/*`, `bash-sandbox/partial-landlock`, `hmr-config`, `gen-third-party-notices`, and `oxlint-contract` specs also flake intermittently under full-suite concurrency on node 26; all pass in isolation and in the CI matrix.
