...
完成 2026-08-18 → 2026-08-28 的上游同步（`daypaw-sync/2026-08-28`，上游 `deepseek-ai/deepseek-harness@cd5ef81481`）恰好撞上上游的 client 架构重写：上游 client 包为 `dsh-api-{session,settings,workspace}-controller`，配拆分的 `dsh-client-store` 与 `dsh-client-ui-*`，顶层 `examples/` 目录消失，交互模型为 Remote Event 瀑布 + 声明合并 StandardProps。fork 壳线（#54–#62）建立在旧架构上，因此 49 个冲突文件、约 100 处测试类型错误、5 个组装快照文件都需要移植而非合并。
...
- **fork UI 壳线移植**（#54–#62 新增的全部 fork 壳文件）：`ctx.slots` → ui-renderer；会话标准件 → `useChat`/`useSession`/`useSessionPendingInteraction`（声明合并）；pending 审批 → ui-approval 的 `PendingApproval` 类（`answer()` 通道）；`IApiClient` → `ctx.remote` 命名空间（含子路径 inject）；`host.describe` → `session/modelCatalog`；名册行不携带 `agentPreset` 字段；conversation 座位 session-maybe → session 域。
...
- **web-app roster**：session-reference、file-reference-local、subagent-model-selection-settings、session/settings/workspace 三 controller、ui-renderer、ui-session、ui-approval、ui-chat。
...
- **文档**：doc-sync 全链绿（2026-08-28 配对规则收紧——687 个 zh note 补 switcher/链接对齐、fork 语料排除入 manifest、生成文档 zh 同步、AGENTS 预算上限 1990）。
...
- fork UI 壳线用新上游 client 架构；后续该区域的 sync 是增量。
- `ensureSymlink` 不在登记在册的 core touch 内；fork 消费上游公开的 `healProfilesModuleFallback`。
- 已知环境性基线红（非本 sync 引入）：`packages/session/session-projection-cache/tests/cache.spec.ts`（cold-read 与 write-policy 用例）在 node 26 下偶发失败——在同步的上游 commit 上，node 26 下红、node 23.11 与 node 24（CI）下绿。`hooks/*`、`bash-sandbox/partial-landlock`、`hmr-config`、`gen-third-party-notices`、`oxlint-contract` 等 spec 在全量并发下亦偶发；单独运行与 CI 矩阵均绿。
