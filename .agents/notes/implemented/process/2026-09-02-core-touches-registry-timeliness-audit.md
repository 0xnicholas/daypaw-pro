...
The audit checks both directions against the 2026-08-28 upstream checkpoint (`cd5ef81481`): every path a row names must exist with its described edit still present, and every fork-modified upstream file must be claimed by a row or that row's connected-file set. The registry rows below carry the corrections:
