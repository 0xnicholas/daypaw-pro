...
ADR 0011 ruled that v1 customer delivery is two self-contained npm packages (`@daypaw/cli`, `@daypaw/sdk`) built from vendored-closure tarballs, and the spike proved the route end to end. The release machinery is owned by `scripts/release/daypaw.ts` and the release-daypaw workflow, satisfying the repo's package gates — which assume every `packages/*/*` package is private, workspace-wired, and carries an invariant companion — and neutralizing three pnpm-deploy hazards the spike surfaced.

## Decision

- **`packages/daypaw/cli` carries a manifest whose `dependencies` list the whole vendored closure the runtime resolves**: the deploy-root manifest (dependencies = `@deepseek-ai/dsh` plus every peer-only Service Definition package legacy deploy would drop, mirroring the `python/sdk-runtime` precedent), the `bin.mjs` shim that seeds the daypaw profile ([the seeding note](../architecture/2026-08-22-daypaw-profile-template-seeding.md)) before importing `@deepseek-ai/dsh/lib/bin.js`, and the gate-mandated explained-empty invariant companion (retired with the whole fork companion set by [the omission note](../simplification/2026-09-03-omit-daypaw-invariant-companions.md)). It and `@daypaw/sdk` are the family's two publishable exceptions (real `0.x` versions, `publishConfig.access: public`); `check-workspace-constraints` carries a named exemption set, and the sdk's npm-range peers are exempted from the workspace-protocol rules. Every exemption is registered in `docs/fork/CORE_TOUCHES.md`.
- **The sdk's consumer-supplied singletons are npm-range peers**: `@deepseek-ai/cordis` `~4.0.1` and zod `^4.4.3` moved out of dependencies so consumers share one zod (`@deepseek-ai/dsh-invariants` was a third peer until its companion was omitted — [the omission note](../simplification/2026-09-03-omit-daypaw-invariant-companions.md); `@deepseek-ai/dsh-attachment` joined as a type-only third peer after upstream moved it out of dsh-llm's peers — [the attachment note](../bug-fix/2026-09-10-sdk-attachment-consumer-peer.md)). The sdk re-exports `DurableEngine` so consumers never `createRequire` into the vendored closure; `@daypaw/engine`/`@daypaw/store` stay private and ship only inside the sdk tarball.
- **`scripts/release/daypaw.ts` owns the pipeline** (`pnpm run release:daypaw`): `pnpm run release:daypaw` builds both faces, then for each package runs `pnpm deploy --legacy --prod`, and copies the staging tree by content (deploy hardlinks workspace files, so in-place edits would write through to the repo), a closure check that fails the release when any dependency or non-optional peer of a staged package is missing, restore of dropped packages from their repository sources, manifest rewrite (`workspace:` → staged versions; cordis peers → `~` ranges; strip devDependencies; `bundleDependencies` = the staged closure), `npm pack`, then two smokes: a clean-prefix global install of the CLI booted with `--profile daypaw` to the no-API-key line, and a registry-installed SDK consumer that typechecks under NodeNext and runs one workflow to a typed result. `--publish` publishes; the default is pack-only.
...
- **Bundling cordis inside the sdk tarball too** — rejected: consumers mount the engine into their own Cordis app, and a second bundled cordis copy breaks the service singleton; the fork's vendor/ is byte-identical to upstream's published releases, so npm ranges are safe. dsh-invariants ships inside the sdk tarball through the vendored closure members' peers.
