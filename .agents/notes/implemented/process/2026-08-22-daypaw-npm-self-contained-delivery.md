...
- **The sdk's consumer-supplied singletons are npm-range peers**: `@deepseek-ai/cordis` `~4.0.1` and zod `^4.4.3` moved out of dependencies so consumers share one zod (`@deepseek-ai/dsh-invariants` was a third peer until its companion was omitted — [the omission note](../simplification/2026-09-03-omit-daypaw-invariant-companions.md)). The sdk re-exports `DurableEngine` so consumers never `createRequire` into the vendored closure; `@daypaw/engine`/`@daypaw/store` stay private and ship only inside the sdk tarball.
