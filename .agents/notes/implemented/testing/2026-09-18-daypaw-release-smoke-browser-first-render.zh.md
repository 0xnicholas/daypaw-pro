...
被伺服的壳的浏览器构建从不执行。组装金样(必需 `daypaw-web-goldens` 门)在 jsdom 里 boot 的是 tsdown `lib/client.js` 产物;`apps/daypaw-web/vite.config.ts` 的 vite 组合——源面 alias、`node:module` stub、`process.versions.node: "0.0.0"` define、vendor 分块——只被当 HTML 文本验证:cli 冒烟的 dist 探针就是对一次 fetch 的 `body.includes('daypaw')`。Playwright 只装给上游壳的 advisory、main-only 浏览器 replay。于是 browserization define 断掉时 build 绿、伺服绿、渲染空白,没有任何车道可见([#121](https://github.com/0xnicholas/daypaw-pro/issues/121),架构评审二候选②)。
...
- **浏览器自己走令牌交换**(goto 打印出的 launch URL,断言落在裸源);既有手写 fetch 探针保留——它以精确失败文钉住 303+cookie 线契约,两者断言不同事实。
...
- 活树探针(define):删 `process.versions.node` define——vite build 仍绿,冒烟在标记等待处红(`getByText('通用助手')` 超时):正是 build 绿/伺服绿/渲染空白这一类,自此有主。
- 活树探针(alias):删 `node:module` alias **不**红。`ModuleLoader.fromInternal()` 在 `"0.0.0"` define 下早于任何 node:module 调用点返回,且本 bundler 对 node 内建软外置,故 alias 不承重首渲染。实证发现:门的契约是"凡杀死被伺服渲染的 browserization 断让 PR 红",不是"每个具名组合行都承重"。
...
- 标记钉住 starter 业务名:重命名播种 starter 或黄卡链路会触碰冒烟常量,超时红文同时指名标记与等待。
