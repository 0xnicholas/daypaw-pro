...
- Main pushes get CI through a new fork-owned workflow, `.github/workflows/ci-daypaw-main.yml`, split by determinism. A required job runs the additive `ci-daypaw-hosted` aggregate (`scripts/run-gates.ts` core touch): every deterministic gate of upstream's primary aggregate — static checks, typert contracts, typecheck, lint, duplication, node smokes, doc-sync, module graph, knip, build, publint, node-next types, built-package invariants, bin smoke — plus the web browser snapshot lane. An advisory job (`continue-on-error`) runs the two full-suite test lanes, coverage and recorded-session snapshot replays: their timing-sensitive process tests measure the host on 4-vCPU hosted hardware rather than the code (upstream runs them on 16-core enterprise runners and keeps its own hosted serial reference disabled), so they report per push without gating it. Restoring them as gates requires enterprise-class runners or upstream stabilization. Four real defects were caught by the required gates during the lane's first runs.
