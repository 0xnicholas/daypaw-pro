...
- Main pushes get CI through a new fork-owned workflow, `.github/workflows/ci-daypaw-main.yml`, split by determinism. A required job runs the additive `ci-daypaw-hosted` aggregate (`scripts/run-gates.ts` core touch): every deterministic gate of upstream's primary aggregate — static checks, typert contracts, typecheck, lint, duplication, node smokes, doc-sync, module graph, knip, build, publint, node-next types, built-package invariants, bin smoke — An advisory job (`continue-on-error`) runs the three full-suite lanes — coverage, recorded-session snapshot replays, and the web browser snapshot (its browser-timing tests proved marginal on hosted hardware too, passing and failing across runs after Playwright was provisioned): their timing-sensitive tests measure the host on 4-vCPU hosted hardware rather than the code (upstream runs them on 16-core enterprise runners and keeps its own hosted serial reference disabled), so they report per push without gating it. Restoring them as gates requires enterprise-class runners or upstream stabilization. Four real defects were caught by the required gates during the lane's first runs.
- The lane pins `DSH_GATE_CONCURRENCY=1` only — gates serialize so a 4-vCPU runner never juggles two lanes (the first run failed exactly there), while each lane keeps its natural intra-suite parallelism, matching the upstream pull-request lane's shape. The standby's full serial pinning targets its weaker single-purpose VMs and starved timing-sensitive process tests on hosted hardware and, after serialization, on the missing hosted environment the standby also prepares (Playwright Chromium, bubblewrap with unrestricted user namespaces), and the same run surfaced one real violation the dead lanes had let through — `agents-dir.ts`'s dynamic factory import assigned `any` (`no-unsafe-assignment`); the import is now structurally typed at the file boundary. The snapshot lane surfaced a third: the corpus contract reserves the `.snapshot.ts` suffix repo-wide for its five registered recorded-session adapters, and six fork test files carried it — the daypaw-web golden replays and the skeleton's CLI replay — so they renamed to `tests/**/*.golden.ts` per `docs/testing.md`'s rule that owner-local expected output never uses the reserved suffix. The skeleton's replay later left the `.golden.ts` suffix for the default lane as `agent.spec.ts` ([dead-lane fix](../bug-fix/2026-09-09-skeleton-agent-replay-dead-lane.md)). The pwsh real-shell test's harness timing widened (idleSilence/handoffGrace 300→2000ms via its first-class timing parameter): cold pwsh on a hosted 4-vCPU runner boots its profile silent past the default bound, letting `inferred_idle` beat `stdin_read`.
