...
- **The shared gates adopt the upstream conditional shape**: `package-invariants` accepts a companion-less package with a README reason and a package whose companion carries the `No runtime invariant:` marker, and rejects export/publication/build wiring left behind after omission; `verify-built-package-invariants`, the Vitest host (`test-invariants.ts`), and `check-workspace-constraints` skip absent companions. The marker branch is the one transitional fork delta against upstream's gate version, which does not contain it. ADR 0007 §2's companion convention and the ADR 0011 sdk-peer addendum carry the current rule, and the release script drops the retired dsh-invariants peer from `SDK_EXTERNAL_PEERS` and the sdk smoke consumer.
...
- Invariant coverage lives in the service-only test roots plus the exhaustive topology test over every published companion.
...
- `scripts/package-invariants.ts` is registered in the CORE_TOUCHES row for the additional marker branch, which upstream's gate version does not contain.
