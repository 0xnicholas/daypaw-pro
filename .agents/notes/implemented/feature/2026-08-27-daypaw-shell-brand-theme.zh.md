...
spec 05 §7 定产品壳的视觉品牌（裁决 [#48](https://github.com/0xnicholas/daypaw-pro/issues/48)）：daypaw 文字标、暖橙 accent 叠暖中性底（「帮手的桌面」，与上游 dev 壳一眼可区分）、单一中偏低密度、亮色调色板开箱默认、暗色经既有 `ui-theme` 偏好可达。壳整包组合 `ui-theme`，因此这项工作要在不 fork 主题机制的前提下回答两个问题：品牌值住在哪里才能不让任何硬编码色值或间距散进组件；「亮主题默认」如何与上游 `DEFAULT_PREFERENCE: 'system'` 相容。
...
- **fork 铸造的 token 带 `--dp-` 前缀。** 密度尺度为 `--dp-space-0..7`（2–24px）；`--dsw-*` 仍归上游所有。四个 fork UI 包的 CSS Module 只经这些 token 引用间距（离档值向上取整：10→12、14→16、18→20——spec §7 把精确间距值定为实施期工作）；色彩只走别名。`brand-tokens.ts` 是品牌裸值的唯一家。
- **亮默认是本工作唯一的上游 core touch。** `ui-theme` 的 `DEFAULT_PREFERENCE` 为 `'light'`（AppearanceRow store 的 init 读该常量），登记于 [CORE_TOUCHES.md](../../../../docs/fork/CORE_TOUCHES.md)。ADR 0001 阶梯在此之前穷尽：settings namespace 属主注册（第二次 `register` 抛错，fork 加不了 `base` 层）、插件没有 config 钮、把 `preference: 'light'` 种进 `$DSH_HOME/settings.yaml` 会写下用户从未做过的选择覆盖，且该文档与 dsh CLI 共享。
- **主题行是 light/dark/system 下拉；壳没有密度行。** 通用分区的主题行是经 `ctx.theme` 的 light/dark/system 下拉（apply 闭包镜像 store 走页面 hooks 通道，写入走服务唯一写入口）。通用分区没有密度行：壳只发一种密度，「即将上线」背后没有东西可上线；尺度改由品牌层承载。
...
- **以品牌 bundle 内的 CSS 文件承载密度尺度**——否决：品牌值将住在两个家（样式表 + 主题层）；覆盖层保住唯一来源、在组件绘制前生效、并随插件 fiber 拆除。
