...
Issue #52（spec 05 §5）声明产品壳 agent 目录经[定义注册表只读视图](2026-08-25-daypaw-definition-registry-view.zh.md)读出的展示元数据：业务名 + 描述（#40 的下限集合）。`EngineDefinition.display` 承载展示元数据；`defineAgent` 可选声明它，什么都不声明的定义不带它注册。

## Decision

- **`DefineAgentOptions.display?: DefinitionDisplay`** —— `defineAgent` 增加可选展示对（`title` + `description`），复用引擎的载体类型，不另立 SDK 本地孪生。`AgentDefinition` 承载它，`ctx.durable.listDefinitions()` 随身份报出所声明的 display。
