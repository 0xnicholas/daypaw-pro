...
ADR 0003 fixed five ctx primitives, but `ctx.spawn` stayed undefined: spec 02 §2 read "spawn semantics undesigned" (ADR 0010 §4 excluded it), and the engine README promised it would "land on demand with its state machine" ([ticket #125](https://github.com/0xnicholas/daypaw-pro/issues/125)).
...
- **Reserved slot keys count per scope** — the top level keeps `sleep:<n>` / `spawn:<n>`, a call inside a step keys under that step (`<stepKey>/sleep:<n>`), and each scope has its own counter, so a skipped step can no longer shift an outer call's occurrence. `ctx.sleep` moves to the same rule in the same change.
- **Presentation and the wire stay out of this change** — `WireRun` gaining `parent_step_key`, the shell's spawned-children section, and the board's list row are the shell's work; the ledger now carries the fact they need.
...
- Cancellation now means "this run and its unfinished subtree". That closes a hole older than spawn — a cancelled parent's awaited child kept running — and gives an operator a stop path for a child that outlived its parent.
- `sleep` keys change shape for nested calls only: top-level `sleep:<n>` rows keep their keys, and a sleep inside a step records `<stepKey>/sleep:<n>`.
...
- Coverage stays per-file 100%: `engine/tests/cancel-cascade.spec.ts` carries four cases (deep cascade, finished descendant untouched, the terminal-parent orphan sweep, the caller-signal path) and `sdk/tests/spawn.spec.ts` seven (derived id and linkage, re-drive attach without a duplicate, failure not reaching the parent, parent cancellation, a cancelled body refused a spawn, unbound definition loud, both definition families at the type face); `sleep.spec.ts` gains the scoped-key case and the misalignment regression, and the fault-injection suite gains the two subtree-walk windows. Mutation probes confirmed the tests fail when the cascade is disabled, when slot keys revert to flat counting, and when the driver-side guard is removed.
