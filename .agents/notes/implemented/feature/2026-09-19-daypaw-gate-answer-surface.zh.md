...
gate 的值契约是任意 JSON——引擎持久化它的 `schema_json` 供表单渲染（ADR 0002 §3）——所以作答面需要一个调用方写出的值，而不是一对固定按钮。
...
- **写面是 `@Remote('resolveGate')`。** 浏览器省略 `source`，host 记 `'manager'`（ADR 0002 §3 的 Manager UI 档），SDK 调用方照旧自带 source。边界类型 `WireGateSettlement` 把批准值定为 `Json`：Remote 参数不能携带无约束的 `unknown`；host 缝保留 `GateSettlement` 更宽的值。first-wins 保持可观察——`false` 表示该 gate 已被他人、超时或取消结算。
...
- **引擎保持静默 first-wins 语义。** 未知 run 或已过期的 gate 返回 `false` 而非抛错：那是这条缝的既定契约，卡片把它读作「已被作答或已超时」。

## Alternatives considered

**这一步就做 schema 驱动的表单渲染。** 它要把 promise 的 `schema_json` 上 wire 再加一个表单渲染器，而 ADR 0002 §3 已经把那一面交给 Manager UI；在它落地前 JSON 框服务技术用户面，平台上新任务弹窗承接任意定义契约用的也是同一种做法。
...
**一对不带值的同意/拒绝按钮。** 声明了契约的 gate 都会拒收空批准（首个真负载的 gate 要 `{approve:true}`），于是那对按钮对现存 gate 一发一个准地失败。
