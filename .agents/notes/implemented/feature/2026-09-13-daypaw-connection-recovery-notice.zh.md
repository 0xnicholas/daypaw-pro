...
落 assembled golden（`apps/daypaw-web/tests/connection-recovery.golden.ts`——offline 事件 → 断线药丸 → 药丸点击 → 恢复确认 → 回落，经 fixture 传输上的真实 `ConnectionController` 全程）暴露出 **daypaw 浏览器 boot 在 HEAD 上是坏的**，且与本改动无关：2026-10-09 sync 漏了两项对账。其一，上游 `11d6bd05f3` 让 `ui-chat`/`ui-skill`/`ui-reference` 硬等待新 `sidebarRight` 服务（`api-workspace-files` 硬等待 `resources`），而 fork 的 `@daypaw/web-app` roster 镜像层没收供主行——boot 断言 4 条目 pending 失败，且没有 CI 车道跑 daypaw goldens 所以无人察觉。镜像层现在收 `resources` + `ui-sidebar-right` 两行（后者只作服务宿主与 `'rightbar.session'` 声明者；fork 的 `TaskDetail` 以 -1 优先级遮蔽其 occupant，沿用既定的遮而不删模式；文档/文件 tab 两行仍是 fork 有意裁剪）。其二，ui-inbox 两处遮蔽席的注册是只等 `layout` 服务的直接 `slots.register`，而 `'main.conversation'`/`'rightbar.session'` 由 ui-conversation/ui-sidebar-right 声明，其 fiber 激活与该门无序——代码自己的注释早已承诺「registrations depend on each seat through `slots.inject()`」，注册现在照做。`'sidebar'` 席保持直接注册（ui-layout 在提供 `layout` 的同一 effect 里声明它，缺席时失败保持大声）。两项修复登记于 [docs/fork/CORE_TOUCHES.md](../../../../docs/fork/CORE_TOUCHES.md)；`task-progress` golden 的过期 `detailsCol` class 查找随上游改名滚到 `rightbarCol`，预期输出逐字节不变。
