...
Cost: two more client bundles on the boot graph, and two roster rows whose visible payoff waits on a composer surface the fork does not yet render. Bought: any surface that renders the upstream conversation slots — composer rail, message images, trajectory/tool galleries — or the `@` trigger machinery gets the full attachment presentation and file/session candidates with no further composition change, and upstream fixes to either package (the alpha.5 attachment-card window showed these actively maintained) arrive with the sync for free. Spec 05 §4's cluster counts are untouched: ADR 0013 places these two rows outside the #36/#37 reuse-boundary adjudication — `ui-attachment` is in the wholesale-reuse cluster, and `ui-reference` joins the roster without changing those counts.
