...
The assembled golden (`apps/daypaw-web/tests/connection-recovery.golden.ts`) covers the offline event → outage pill → pill click → recovery confirmation → settled circuit through the real `ConnectionController` over the fixture transport. It is also the boot assertion's executor, so the `@daypaw/web-app` roster must satisfy the service constraints upstream `11d6bd05f3` introduced: `ui-chat`/`ui-skill`/`ui-reference` hard-wait a `sidebarRight` service and `api-workspace-files` waits on `resources`. The mirror carries `resources` + `ui-sidebar-right` (the latter as service host and `'rightbar.session'` declarer; the fork's `TaskDetail` shadows its occupant at priority -1, the established shadow-not-remove pattern; the document/file tabs stay deliberate fork trims). Second, ui-inbox's shadowed-seat registrations were direct `slots.register` calls gated only on the `layout` service, but `'main.conversation'`/`'rightbar.session'` are declared by ui-conversation/ui-sidebar-right whose fiber activation is unordered against that gate — the registrations depend on each seat through `slots.inject()`, which resolves a seat through its own provider. The `'sidebar'` seat keeps its direct register (ui-layout declares it in the same effect that provides `layout`, so the absent-frame failure stays loud). Both fixes are registered in [docs/fork/CORE_TOUCHES.md](../../../../docs/fork/CORE_TOUCHES.md); the `task-progress` golden's stale `detailsCol` class lookup rolled to the upstream-renamed `rightbarCol` with byte-identical expected outputs.
