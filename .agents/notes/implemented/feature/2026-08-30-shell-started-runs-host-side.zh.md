...
- **注册源 = `daypaw/agents/` 目录扫描,注入式工厂**(ADR 0012)。`loadAgentFiles(ctx, dir)`(`@daypaw/sdk/agents-dir`,新增子路径导出)返回 `dir` 下 `.mjs`/`.js`/`.ts` 文件声明的定义:每个文件的 default 导出是接收 SDK 命名空间的工厂——`export default ({ defineAgent, defineWorkflow, z }) => 定义`(或定义数组)——经 `bindAgent`/`bind` 注册。文件零裸导入:交付安装把 daypaw 族链接在 `$DSH_HOME/profiles/node_modules`,工作区的父级解析够不到,自装则会在进程内装入第二份 SDK/engine 拷贝。缺目录 = 合法空名册;坏文件(导入失败、无 default 工厂、抛错、产物非定义)失败响亮指名文件。组合住在 `@daypaw/web-app` 胶水:新增 `agentsDir` 配置(默认 `daypaw/agents`,与 ledger 路径同为 cwd 相对),在 `ctx.inject(['durable'], …)` 内装载——async 插件 fiber,其拒绝使 Loader boot 失败,即失败响亮通道;不装 engine 行的组合有意不服务名册。
...
- 壳宿主的注册表供出弹窗与目录所读的名册（`durable/listDefinitions`）；preset 退居上游兼容层，与 agent 目录名册分开。
...
- 壳侧 UI(弹窗读定义、看板切 run 主源、sessionId≡runId 对话投影、#56 投影退役)归 #67;daypaw web 快照车道的 golden 刷新也归 #67。宿主侧覆盖的行为：起 run、装载 agent 目录、web-app roster 接线与 starter 播种。
