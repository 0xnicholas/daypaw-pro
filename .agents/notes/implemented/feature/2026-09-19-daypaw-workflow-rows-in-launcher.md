...
- **A definition whose input kind is `null` uses the JSON input box and the JSON send path.** The renderer distinguishes "no row selected" from "row whose input kind is `null`", so such a row renders the JSON box, matching what `composeInput` sends: the engine inserts the value as given, and the definition's own input contract validates it when the run starts.
