...
const MANIFEST_BIN_ALLOWLIST = new Map<string, ManifestBin>([
  ...
  // Fork product CLI: self-contained bin over the vendored launcher (ADR 0011
  // delivery shape — the daypaw command IS the application front door).
  ['packages/daypaw/cli/package.json', { daypaw: 'bin.mjs' }],
])
...
const EXECUTABLE_SOURCE_ALLOWLIST = new Map<string, string>([
  ...
  ['packages/daypaw/cli/bin.mjs', 'daypaw product CLI (self-contained, ADR 0011)'],
