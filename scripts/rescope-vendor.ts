...
const GENERIC_SKIPS: readonly GenericSkip[] = [
  ...
  // The inspector bridge names its runtime-tree snapshot topics `cordis/*`
  // (wire ids, not package references) and the query/tree specs assert them.
  ...
  // `cordis/*` is the extensions event domain and the dynamic-Cordis stack's
  // wire-event namespace: events the host runner emits on the Context bus
  // (`cordis/dynamic-package`, `cordis/request-run`, `cordis/inspect-query`, …)
  // and every consumer subscribes to by the literal name. The spelling is
  // indistinguishable from a subpath import, so each file naming one is
  // skipped wholesale; the generated catalogs and `EVENT_SCOPE_PAGE` key those
  // same event scopes.
  ...
  // keys, not package names. The UI locale namespace and input-trigger source
  // id are product keys.
