...
| `runtime` | React-free 对象层：Session/Workspace 对象、事件窗、流式累积、projection、ConversationNode 装配机全部在此——重写表现层后仍由它供数；术语「Session/Workspace」不出现在 UI 文案义务里。 |
...
| `web-react` | ctx↔React 粘合（SlotRenderer、SessionProvider、hook 绑定），业务包本就不依赖它，纯壳内机械。 |
...
| `schema-form` | settings 编辑器的 schema/draft 模型层，无 React 无渲染；产品壳设置面的草稿语义直接受用。 |
