...
| --- | --- | --- | --- | --- | --- | --- |
| `ctx.durable` | `seam` | `daypaw/engine` | `engine` | `sdk` | - | fork 自有编排器：run 生命周期、step 去重续跑与 boot 扫描复活，落在 @daypaw/store ledger 上；@daypaw/sdk facade 是其类型化消费方（packages/daypaw/engine/README.md）。 |
