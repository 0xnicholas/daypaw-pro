...
| `tsconfig.client.json` | references 新增 `apps/daypaw-web` 一行（client 聚合首个 fork 条目） | 同上；`apps/daypaw-web` 是 client 侧构建入口，只能进 client 聚合 | 否（fork 私有包） | F（产品壳脚手架） |
...
| `knip.json` | workspaces 新增 `packages/daypaw/cli`（entry/project 同规约 + `ignoreDependencies: @deepseek-ai/.+`）、`apps/daypaw-web` 与 `packages/daypaw/web-app`（closure manifest 豁免） | cli 与 web-app 是闭包 manifest 包，dependencies 列表刻意不被代码引用（同 apps/cli、packages/bundle/web-app 先例）；daypaw-web 同 apps/web 形状减 e2e 道 | 否（fork 私有包） | D（客户交付）/F（产品壳脚手架） |
