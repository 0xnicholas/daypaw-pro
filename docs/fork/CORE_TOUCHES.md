...
| `tsconfig.host.json` | references 新增 `packages/daypaw/{store,engine,sdk}` 三行（host 聚合，daypaw 无 client 侧包） | 同上；项目引用无通配形式，只能逐包登记 | 可提（同上） | C（走骨） |
| `scripts/check-workspace-constraints.ts` | release-member 目录正则排除 `packages/daypaw/` 组（负向前瞻） | 上游 npm-public 化后 `packages/*/*` 一律要求可发布，与 ADR 0001「@daypaw 独立 0.x private」相抵；fork 私有姿态优先 | 可提（上游无 daypaw；规则形状属上游发布流程） | C（走骨） |
