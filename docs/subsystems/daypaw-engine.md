 ...
 * starter-text rule {@link startRun} applies; `@daypaw/sdk/wire` owns
 * the rule's declaration. The journal records the input the consuming
 * body expects. Fails loud when the run is unknown, its
