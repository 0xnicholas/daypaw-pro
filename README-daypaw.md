...
客户交付（[ADR 0021](docs/adr/0021-bespoke-delivery.md)，取代 ADR 0011 §1 的渠道裁决）：交付物是**按客户定制的构建**，通道是 `pnpm run release:daypaw` 产出的 tarball（`dist-daypaw/*.tgz`），不公开发布；定制面限 T1（客户自己的 profile 行 / `daypaw/agents/` / 设置）与 T2（客户自有插件包），T3（改 fork 源码）排除；key 由客户自带。公开 registry 路径休眠（发布向导 `bash scripts/release/publish-daypaw.wizard.sh` 保留待用）。本机跑完 `release:daypaw` 要补跑一次 `pnpm install`：pnpm deploy 的 hoist 残留会让下一次 `pnpm run` 要求交互式 purge（CI 每次全新检出，不受影响）。
