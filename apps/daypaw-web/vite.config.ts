...
const STANDALONE_ERROR = 'apps/daypaw-web is not a standalone application: bare Vite cannot inject window.__DSH_BOOT__. '
  + 'Only the daypaw CLI injects the boot manifest when it serves the built shell; from a repository checkout, '
  + 'run `pnpm run build` and use the shell through the daypaw CLI.'

/** Fail before a Vite dev or preview server can expose the boot-manifest-free shell. */
function rejectStandaloneServe(): Plugin {
  return {
    name: 'daypaw-reject-standalone-web-serve',
    config(_config, env) {
