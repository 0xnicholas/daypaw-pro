...
const BUNDLE_LAYERS = [
  ...
  {
    manifest: join(REPO_ROOT, 'packages/daypaw/web-app/package.json'),
    patch: join(REPO_ROOT, 'packages/daypaw/web-app/cordis.patch.yml'),
  },
] as const
 ...
 * Chinese pinned before boot so the daypaw golden text stays deterministic
 * (the fork product copy is zh-primary), the observers and frame callbacks
 * jsdom lacks, and a full reset of the document, the boot globals, and the
 * injected plugin styles afterwards.
 */
export function installAssembledBootEnv(): void {
  ...
  beforeEach(() => {
    ...
    // English here (the fork goldens render the en dictionary).
