...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'inbox.agents.page',
    kind: 'single',
    scope: 'session-maybe',
    summary: 'The Agents catalog page: the single occupant draws the whole catalog surface (cards + detail) inside the middle column.',
    doc: 'The Agents catalog page: the single occupant draws the whole catalog\nsurface (cards + detail) inside the middle column. An absent occupant\nfalls back to the owner\'s placeholder.',
    registerOptions: [],
    ownerProps: [],
    ownerPropsReferences: [],
    standardProps: [
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'main.conversation\' (@daypaw/ui-inbox), so it exists while that entry is mounted',
    occupants: [
      '@daypaw/ui-agents AgentsPage',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'inbox.agents.page\', () => ctx.slots.register(\n      { name: \'inbox.agents.page\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/daypaw/ui-inbox/src/client/contract.ts:173',
  },
  {
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'rightbar.session\' (@daypaw/ui-inbox), so it exists while that entry is mounted',
    occupants: [
      '@daypaw/ui-tasks DetailBody',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'inbox.detail.body\', () => ctx.slots.register(\n      { name: \'inbox.detail.body\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/daypaw/ui-inbox/src/client/contract.ts:182',
  },
  {
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'sidebar\' (@daypaw/ui-inbox), so it exists while that entry is mounted',
    occupants: [
      '@daypaw/ui-tasks NewTaskDialog',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'inbox.new-task.dialog\', () => ctx.slots.register(\n      { name: \'inbox.new-task.dialog\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/daypaw/ui-inbox/src/client/contract.ts:153',
  },
  {
    key: 'inbox.settings.page',
    kind: 'single',
    scope: 'session-maybe',
    summary: 'The 设置 page content: the single occupant draws the whole settings surface inside the middle column.',
    doc: 'The 设置 page content: the single occupant draws the whole settings\nsurface inside the middle column. An absent occupant falls back to the\nowner\'s placeholder.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'main.conversation\' (@daypaw/ui-inbox), so it exists while that entry is mounted',
    occupants: [
      '@daypaw/ui-settings SettingsPage',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'main.conversation\' (@daypaw/ui-inbox), so it exists while that entry is mounted',
    occupants: [
      '@daypaw/ui-settings ApiKeyCard id \'api-key\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'main.conversation\' (@daypaw/ui-inbox), so it exists while that entry is mounted',
    occupants: [
      '@daypaw/ui-tasks ConversationView',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'inbox.workspace.conversation\', () => ctx.slots.register(\n      { name: \'inbox.workspace.conversation\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/daypaw/ui-inbox/src/client/contract.ts:167',
  },
  {
    key: 'inbox.workspace.conversation.inspector',
    kind: 'list',
    scope: 'session',
    summary: 'The conversation seat\'s inspector view ring: session-scoped list slots whose owner share is ui-conversation\'s view-ring face (the retargeted trajectory ledger registers here — the cordis row\'s `viewSlot` config points its registration at this key, ticket #105).',
    doc: 'The conversation seat\'s inspector view ring: session-scoped list slots\nwhose owner share is ui-conversation\'s view-ring face (the retargeted\ntrajectory ledger registers here — the cordis row\'s `viewSlot` config\npoints its registration at this key, ticket #105). The seat renders the\nring only while the 检查器 tab is active.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'inbox.workspace.conversation\' (@daypaw/ui-tasks), so it exists while that entry is mounted',
    occupants: [],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'root',
    summary: 'One inbox group\'s task list: the single occupant renders the owner\'s projected rows.',
  ...
  },
  {
    ...
    occupants: [
      ...
      '@daypaw/ui-inbox WorkspaceSwitch',
    ],
    replaceRisk: 'shadows-shipped-ui',
