...
export class ClientSessions implements ISessions {
  ...
  /**
   * Catalog metadata and local reference-source projection (list RPC + host
   * stream increments; re-pulled on reconnect and after a removal frame,
   * which reconciles a persisted-but-disposed session back in) — the
   * useSessions standard feed, current included.
   */
