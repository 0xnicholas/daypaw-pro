...
import { bind } from '@daypaw/sdk'
import { createSkeletonWorkflow } from './index.ts'
...
const skeleton = createSkeletonWorkflow({
  record: async (name) => { await appendFile(effectsPath, `${name}\n`) },
  delay: async () => { if (stepDelayMs > 0) await new Promise(resolve => setTimeout(resolve, stepDelayMs)) },
})
