...
describe('defineAgent compilation replays', () => {
  it('compiles an agent into a durable child run of the workflow', async () => {
    ...
    try {
      ...
      const log = await expectSessionLog(sessions, join(happyDir, 'session.v4.jsonl'))
  ...
  it('revives a SIGKILLed agent run with a synthetic continuation steer', async () => {
    ...
    try {
      ...
      const log = await expectSessionLog(sessions, join(reviveDir, 'session.v4.jsonl'))
  ...
  it('pins the multi-segment model-visible surface of a steered run', async () => {
    ...
    try {
      ...
      const log = await expectSessionLog(sessions, join(steerDir, 'session.v4.jsonl'))
