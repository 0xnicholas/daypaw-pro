...
自包含 daypaw CLI：`npm i -g @daypaw/cli` 安装 `daypaw` 命令——从打进本包的 vendored 运行时闭包直接启动产品壳，无需另装任何 `@deepseek-ai/*` 包。裸 `daypaw` 启动浏览器壳并打印携带启动令牌的 URL 行（`daypaw web: http://127.0.0.1:<端口>/?token=…`）——浏览器认证栅栏拒绝裸源,请打开打印出的 URL;壳应用自己的参数紧随其后（`daypaw --port 8080`、`daypaw --help`）。durable ledger 落在启动目录的 `daypaw/ledger.db`。交付形态：[ADR 0011](../../../docs/adr/0011-customer-self-run-delivery.md)。
