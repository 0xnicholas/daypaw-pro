...
The self-contained daypaw CLI: `npm i -g @daypaw/cli` installs the `daypaw` command, which boots the product shell directly from a vendored runtime closure bundled inside this one package — no other `@deepseek-ai/*` install is needed. A bare `daypaw` boots the browser shell and prints its launch-token URL line (`daypaw web: http://127.0.0.1:<port>/?token=…`) — the browser-auth fence rejects the bare origin, so open the printed URL; the shell app's own flags follow (`daypaw --port 8080`, `daypaw --help`). The durable ledger lands at `daypaw/ledger.db` under the launch directory. Delivery shape: [ADR 0011](../../../docs/adr/0011-customer-self-run-delivery.md).
