/* jscpd:ignore-start -- fork carrier: this file deliberately mirrors
   @deepseek-ai/dsh-web-app's runtime glue with the product-shell diffs ruled
   by docs/spec/05-product-shell.md §4 (dist package name, surface prompt text,
   `daypaw web:` prefix; the DAYPAW_WEB_URL rename is deferred — see the
   DSH_WEB_URL declaration); roster trimming belongs to the later board issues,
   not to scaffold duplication. */
/**
 * @daypaw/web-app — the daypaw browser-surface bundle's runtime glue plugin
 * plus the bundle patch (`cordis.patch.yml`, declared by the `dsh.bundle.patch`
 * manifest field). The plugin owns the browser-surface glue: it resolves
 * the built daypaw frontend dist (workspace knowledge of this bundle, never
 * user config), mounts the `frontend-static` fallback owner over it, registers
 * the harness-source and web-surface prompt sections, the bash-visible web
 * runtime variable, and the URL line. App command-line values arrive through
 * the `webStartup` service expressions in the bundle patch.
 * @module @daypaw/web-app
 */
...
/** This installation's root, from either this package's source or built entry. */
...
/**
 * Environment variable naming the canonical local URL of the daypaw Web GUI.
 * Managed shell variables live in the reserved `DSH_*` namespace: dsh-subprocess
 * strips ambient `DSH_*` from child environments and shell-env rejects other
 * prefixes at registration, so the `DAYPAW_WEB_URL` rename ruled by
 * docs/spec/05-product-shell.md §4 cannot pass this registry without widening
 * that upstream contract; the rename is deferred (package README, Known
 * Limitations) and the upstream name stands meanwhile.
 */
...
/** Model-visible orientation and acceptance boundary for sessions created through the daypaw web surface. */
function webSurfacePrompt(webUrl: string): string {
  const updateContract = 'The client-plugin HMR receiver is active, but this shell has no rebuild watcher wired yet, '
    + 'so every change — client plugins, the apps/daypaw-web shell, and plain packages alike — requires rebuilding the affected Web artifacts and verifying this existing URL after a page refresh. '
  return `You are interacting with the user through the daypaw Web GUI at ${webUrl}. `
    + 'When the user refers to "this page", "this GUI", or "this app" without naming another target, they mean this GUI. '
    + 'The browser provides no implicit DOM, route, or screenshot context. '
    + updateContract
    + 'Starting another server does not update this GUI. '
    + 'The apps/daypaw-web Vite entry builds the shell but is not a standalone application because only the daypaw CLI injects window.__DSH_BOOT__. '
...
function resolveDistIndex(): string {
  ...
  try {
    return require.resolve('@daypaw/web-frontend/dist/index.html')
  } catch {
    /* v8 ignore next 2 -- reachable only on a checkout without a built dist; the test tree builds it */
    throw new Error('web-app: daypaw frontend dist not built; run pnpm run build from the repository root first')
  }
}
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [DSH_WEB_URL]: { description: 'Canonical local URL of the daypaw Web GUI serving this session.' },
        },
