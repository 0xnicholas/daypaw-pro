...
- `WorkspaceSwitch` 以优先级 -1 占据 `'conversation'`（session-maybe scope），遮蔽 ui-conversation 优先级 0 的占位占据者，其声明的席位仍为休眠生态保留。它按选中项切换中栏容器：收件箱分组的任务容器（空态）、Agents 占位页或 设置 占位页。
...
- **Agents 与 设置 页是占位**——次要导航只把中栏切到静态占位容器。
