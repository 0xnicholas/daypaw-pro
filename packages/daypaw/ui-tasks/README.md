...
- **等待你确认 stays empty** — the pending group is a placeholder zero until the approval board ticket (#58) wires pending interactions.
...
- **The conversation flow has no approval cards** — approvals render only as the detail pane's 审批历史 history section; the interactive approval card belongs to the approval ticket.
