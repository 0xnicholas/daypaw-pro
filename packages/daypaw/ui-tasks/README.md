...
- `NewTaskDialog` occupies `'inbox.new-task.dialog'` (single, root; the Modal chrome stays with `InboxNav`). A definition picker over the engine registry's roster (every definition the registry holds, first row preselected — the registry is the one roster, ADR 0012, issue #127): an agent row's business name comes from its declared display title, falling back to the technical name, while a workflow definition carries no display and shows the technical name. The picked definition's `inputKind` rules the input surface: a free-text area for the starter text shapes, a JSON area with inline syntax validation for every other shape — a workflow definition carries no wire face, so its `inputKind` is null and it shares the JSON surface with the json kind. Submit mints one run id and calls `durable/startRun` (start-or-attach): a failed submit keeps the minted id, so a retry attaches to the same run instead of double-creating. The definition's family decides what the owner opens: an agent run's session identity IS its runId and `sessions.open` fails loud on unlisted ids, so the store waits for the list projection to carry the session twin (bounded at `TWIN_WAIT_MS`; a run that failed before creating its session retires the submit inline, run id kept for an attaching retry) and hands the session to `openTask`, whereas a workflow run has no session (ADR 0016) and hands its run id to `openRun`. Both owners navigate, dismiss, and kick a board refetch. Failures land inline on the dialog as generic localized copy — raw host error wording never reaches the screen; a failed roster load retries on the next dialog open instead of wedging.
...
- **The seat steers only unfinished runs** — a settled run keeps the seat closed (任务已结束); a run-less session keeps it live as the plain-chat input (issue #102); a json-kind definition or one without a wire face rejects free text at the boundary and surfaces the inline failure.
