...
- `ctx.sleep(durationMs)` — durable timer: records a deadline for this call's position in the body and parks the run until it passes (the ledger status stays `running`). A recorded wake never waits twice, a recorded deadline is waited for as recorded, and a deadline that passed while no process was running returns immediately, so a crash neither restarts nor extends a sleep; a deadline missed by a dead process is recorded on the next boot scan (spec ch.1 §6).
...
- `ctx.spawn(def, input)` — fire-and-forget child run (ADR 0016): starts the definition (agent or workflow) on a deterministic child runId under a reserved slot key (`spawn:<n>`; `<stepKey>/spawn:<n>` inside a step) with the parent linkage recorded, and resolves to that runId. Awaiting it covers the child's *start*, never its result — no handle, no `result`, no `cancel` come back. The child then lives its own life: a boot scan revives it without its parent, its failure never enters the parent's status or output, and cancelling the parent cancels its unfinished subtree. The definition must be bound with `bind`/`bindAgent` (an unbound one throws), and a spawn the body reaches after cancellation is refused with `RUN_CANCELLED`. Detach with `ctx.spawn`; use `ctx.agent` or a bare `run()` inside `ctx.step` when the result is wanted.
...
- **No concurrency cap, no join** — `ctx.spawn` starts children without a limit (the same exposure as `Promise.all` over awaited children carries), and nothing observes or joins a spawned run: reaching its status or cancelling it needs the ledger or a future primitive (ADR 0016 §2/§5).
- **Spawn classification is not on the wire yet** — the shell's spawn-children section classifies by the `spawn:` prefix of `parent_step_key`, which `WireRun` does not expose; the field rides the shell-side change that consumes it (ADR 0016 §7).
