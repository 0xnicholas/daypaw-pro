...
- `resolveGate(runId, gate, settlement, source?)` — the one settle seam for gates (first-wins): SDK direct calls, Manager UI, and (deferred) webhooks share it. When this process holds the waiter, the value contract validates before the write — an invalid settlement throws and records nothing; a second settler is a no-op returning `false`. A driver waiting in this process resumes immediately; cross-process writes are observed through the driver's `pollMs` poll fallback, and a dead process's waits revive through the boot scan. The method carries the `@Remote` marker, so the browser reaches it as `durable/resolveGate` (ticket #128): the shell's answer card omits `source`, and the host records `'manager'` — ADR 0002 §3's Manager UI entry. Its boundary type `WireGateSettlement` (`types.ts`, the public non-root subpath the Typert analyzer scans) types the approval value as `Json`: a Remote parameter cannot carry unconstrained `unknown`, while the host seam keeps the wider `GateSettlement`. A run suspended on a gate records the gate name in `runs.waiting_gate`, which is what the board's triage and the answer card key off.
- `steer(runId, input)` — append a follow-up segment to an unfinished run (issue #53): throws on an unknown runId, a terminal run, or a locally registered definition that did not opt in with `steerable: true`. The input records as given — in-process callers pass contract-validated values, and a run this process cannot resolve a definition for still records (the consumption side re-validates: the cross-writer defense). Durable before delivery: the segment row records first, then a body parked in this process wakes immediately; elsewhere the parked wait's `pollMs` poll or the next boot scan observes it. Returns the 1-based segment sequence. The method carries the `@Remote` marker, so the browser reaches it as `durable/steer` (the `listDefinitions` precedent).
- `steerText(runId, text)` — append a free-text follow-up segment (ticket #94): the browser follow-up seat's channel. Resolves the run's definition and validates the text through its wire face — the same starter-text rule `startRun` applies — then records through `steer`. Fails loud on an unknown run, an unregistered definition, a definition without a wire face, or a wire contract that rejects free text (a json-kind definition takes none); nothing records on failure. Carries the `@Remote` marker (`durable/steerText`, the `steer` precedent).
...
- `cancel(runId, cause?)` — request cancellation of a run and of everything it started (ticket #74, ADR 0016 §3): the terminal `cancelled` row with the cause is written first, pending gates settle cancelled, and a driver in this process aborts; every unfinished descendant recurses through the same write, so a spawned child that outlives its parent's own work is still reachable, while a descendant that already finished is left alone (cancelling never rewrites what already happened). Idempotent on terminal runs — a run that already ended satisfies the request, its unfinished descendants are still cancelled, and a lingering driver still aborts (a fault between the terminal write and the abort can leave one) — and loud on unknown runIds. Carries the `@Remote` marker (`durable/cancel`, the `steer` precedent).
...
A run drives its body with a step ctx. `ctx.step(name, fn, { key? })` derives the idempotency key `name#occurrence` (or takes an explicit key); a completed step returns its recorded result without re-executing, an unfinished one (re)executes and records — at-least-once execution, exactly-once step commits. Cancellation writes the terminal row first, takes effect at the next step boundary, and cascades to every unfinished descendant (a run that already reached `done` or `failed` never cascades — ADR 0016 §3). Disposal stops driving without terminal writes: unfinished runs stay revivable. Disposal also awaits the ledger open and closes the database before fiber disposal resolves, so no ledger write — WAL sidecars included — can land once teardown returns (issue #115).
...
The step ctx also exposes `runId`, the driver's `signal`, and `slot(kind)` — the reserved-slot allocator behind `sleep:<n>` and the SDK's `spawn:<n>`: its counters live per scope (top level, and each step scope of its own), so a step whose `fn` a re-drive skips cannot shift an outer call's occurrence. While a step's `fn` awaits, `currentStepScope()` returns the ambient scope `{ runId, stepKey }`; a child run started inside it derives its deterministic runId as `<runId>/<stepKey>/<kind>:<name>#<occurrence>` with the parent linkage recorded — a re-driven parent attaches to the child instead of re-opening it (the SDK's `ctx.agent`, `ctx.spawn`, and the bare sub-workflow idiom all ride this).
...
- **Primitive set** — `ctx.step`, `ctx.waitFor`, `ctx.sleep`, the steer channel (`steers` / `awaitSteer`), and the reserved-slot allocator `ctx.slot` behind them are landed; the `agent` and `spawn` faces live in the SDK (ADR 0010, ADR 0016).
