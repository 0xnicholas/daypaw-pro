...
| `resolveProfileDir` / `initProfile` / `loadProfile` / `readProfileManifest` / `writeProfileManifest` / `resolveBundleDir` / `composeEntries` / `healProfilesModuleFallback` / `ensureSymlink(binName, link, target, manages)` / `PROFILE_TEMPLATES` / `DEFAULT_PROFILE_BUNDLES` / `PROFILES_DIR` / `PROFILE_PATCH_FILENAME` | Profile 机制（见 [Profile](#profiles)）；`ensureSymlink` 是 `healProfilesModuleFallback` 逐包应用的幂等单链 heal——错误或悬空链接重指、真实目录抛错（消息携带 `binName`/`manages`）——导出供同类启动器以相同语义 heal 自己的链接 |
