...
function buildAlphaLog(): SessionEvent[] {
  ...
  // The todo_write sample's items: the TodoRow toolview in the flow plus the
  ...
  // unresulted tool/call keeps the paired-command lookup (the window's
  // runningCalls) exercised for the daypaw approval card's details expander.
  // The standing plan (fed by the todo/write snapshot) retires at the NEXT
  // turn/start, so the todo sample must live in the open turn itself: a
  // completed todo turn with the open turn appended after it would retire the
  // plan and empty the dock's strip (the todo surfaces' own coverage with it).
...
export interface FixtureOptions {
  ...
  /**
   * Run the 5s fx-gamma running↔idle flip on the host stream. Off by
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const approvalInvocation = (): FixtureRemoteEventInvocationFrame => ({
    ...
    request: {
      ...
      callId: ToolCallId('fx-call-approval-live'),
  ...
  async function* openRemoteEvents(
    ...
    // Opt-in periodic material: flip fx-gamma every 5s. fx-gamma only; the
    // conversation replay owns fx-alpha's running state. Off by default — an
    // ambient flip races every golden-sampling lane that reads the list; only
    // the fixture spec pins the flip itself.
