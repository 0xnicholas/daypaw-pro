...
export class ClientEntries {
  ...
  start(loader: Loader, manifest: BootManifest): Promise<void> {
    ...
    return this.enqueue(async () => {
      await Promise.all(this.desired.plugins.map(async (row) => {
        await this.create(loader, row.id, row.config)
      }))
      await loader.await()
  ...
  private async create(loader: Loader, id: string, config?: unknown): Promise<void> {
    ...
    const options = { name: id, ...(config === undefined ? {} : { config }) }
